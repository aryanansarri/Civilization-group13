...
import static Models.Units.UnitType.getMilitaryUnits;
...
//                Demographic += " pay: " + citizen.showGoldProductFood();
    ...
    public void addUnitFast(UnitType unitType, Tile tile){
        ...
        if(tile.getCivilianUnit() == null && UnitType.WORKER == unitType)
            new Worker(tile,this.ownership);
        if(getMilitaryUnits() == null && getMilitaryUnits().contains(unitType))
            new MilitaryUnit(this.ownership,unitType,tile);
        if(getMilitaryUnits()== null && UnitType.getSiegeUnits().contains(unitType)){
    ...
    public void establishUnit(Tile tile){
        ...
        if (unitType == UnitType.SETTLER && Tile.getCivilianUnit() == null) {
            new SettlerUnit( getCivilization(),UnitType.SETTLER,tile);
            addingUnit = null;}
        if (UnitType.getSiegeUnits().contains(unitType) && getMilitaryUnits() == null) {
            new SiegeUnit(getCivilization(),addingUnit.getSecond(), tile);
            Tile.getResources().remove(unitType.getPrerequisiteResource());
            addingUnit = null;}
        if (unitType == UnitType.WORKER && Tile.getCivilianUnit() == null) {
            new Worker(tile,getCivilization());
            addingUnit = null;}
        if (getMilitaryUnits().contains(unitType) && getMilitaryUnits() == null) {
