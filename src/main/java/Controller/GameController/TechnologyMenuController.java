...
public class TechnologyMenuController {
    ...
    public String  TechTree(){
        temp = GameDatabase.getGameDatabase().getCurrentCivilization().getCivilizationTechnology().toString();
        return temp;
    }
    ...
    public String pickTechToResearch(Matcher matcher){
        ...
        civilizationTechnology.RequestToReachTechnology(tech,civilizationTechnology.getTechnologyMap().get(tech));
        return "tech set as researching";
    }
    public String showTechInfo() {
        StringBuilder tech = new StringBuilder();
        tech.append("RESEARCHED TECHS\n");
        for (Technology technologyResearched : GameDatabase.getGameDatabase().getCurrentCivilization().getCivilizationTechnology().getPassedTechnology()) {
            tech.append("Technology: ").append(technologyResearched.getType()).append("\n");
        }
        tech.append("\n");
        tech.append("AVAILABLE TECHS\n");
        for (Map.Entry<Technology, Integer> availableTech : GameDatabase.getGameDatabase().getCurrentCivilization().getCivilizationTechnology().getTechnologyMap().entrySet()) {
